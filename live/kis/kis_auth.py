...
key_bytes = 32
# Prefer project-level config folder: <project_root>/config
here = os.path.dirname(os.path.abspath(__file__))
project_root = os.path.abspath(os.path.join(here, "..", ".."))
project_config = os.path.join(project_root, "config")

# Prefer project config if present, otherwise fall back to user home KIS config
if os.path.exists(project_config):
    config_root = project_config
else:
    config_root = os.path.join(os.path.expanduser("~"), "KIS", "config")

# token file path (in chosen config_root)
token_tmp = os.path.join(config_root, "KIS" + datetime.today().strftime("%Y%m%d"))

# Ensure config_root exists and token file can be created
os.makedirs(config_root, exist_ok=True)
open(token_tmp, "a").close()

# 앱키, 앱시크리트, 토큰, 계좌번호 등 저장관리
_cfg = None
dev_path = os.path.join(config_root, "kis_devlp.yaml")
if os.path.exists(dev_path):
    with open(dev_path, encoding="UTF-8") as f:
        _cfg = yaml.load(f, Loader=yaml.FullLoader)
else:
    # fallback to bundled dev config in the project (live/kis/kis_devlp.yaml)
    bundled = os.path.join(here, "kis_devlp.yaml")
    if os.path.exists(bundled):
        with open(bundled, encoding="UTF-8") as f:
            _cfg = yaml.load(f, Loader=yaml.FullLoader)
    else:
        raise FileNotFoundError(f"kis_devlp.yaml not found in {dev_path} or {bundled}")
...
def changeTREnv(token_key, svr="prod", product=_cfg["my_prod"]):
    ...
    try:
        ...
        my_token = ""
    # If a token_key (new token) is provided, prefer it; otherwise keep existing
    cfg["my_token"] = token_key if token_key else my_token
    cfg["my_url_ws"] = _cfg["ops" if svr == "prod" else "vops"]
    # Ensure account is set (default to main stock account if not provided)
    if "my_acct" not in cfg or not cfg["my_acct"]:
        if svr == "prod":
