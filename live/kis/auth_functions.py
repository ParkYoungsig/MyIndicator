...
# Prefer package-relative import; fall back to absolute for direct runs
try:
...
def auth_token(
    grant_type: str, appkey: str, appsecret: str, env_dv: str
) -> pd.DataFrame:
    """
    ...
    if not appkey:
        ...
        raise ValueError(
            "appkey is required. (한국투자증권 홈페이지에서 발급받은 appkey)"
        )

    if not appsecret:
        logger.error(
            "appsecret is required. (한국투자증권 홈페이지에서 발급받은 appsecret)"
        )
        raise ValueError(
            "appsecret is required. (한국투자증권 홈페이지에서 발급받은 appsecret)"
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                "API call failed: %s - %s", response.status_code, response.text
            )
            return pd.DataFrame()
...
def auth_ws_token(
    grant_type: str, appkey: str, appsecret: str, env_dv: str, token: Optional[str] = ""
) -> pd.DataFrame:
    """
